...
- C2B.2B Movies bot migration (2026-09-26): **BLOCKED, recovery prepared.** The Movies bot is `@veloramovies_bot` (display name `velora_movies_bot`). Its cloud `logOut` succeeded at 07:49:08Z, but the local server aborted on its first login: TDLib cannot stat its binlog after an open-file rename on the Windows bind mount. Bot API state now uses the named volume `velora-telegram-bot-api-state` (probe and recreate verified). `TELEGRAM_BOT_API_LOCAL_BOTS` now gates bots one at a time, and each listed bot must prove its configured id and exact username through `getMe` before any call. The Movies token was exposed in an operator session. It has since been rotated (the old token returns 401), and its failed session directory has been deleted. Series is still on the cloud, uploads remain disabled, and hosted has no channel rows or checkpoints. Record: `docs/PHASE_C_INGESTION_DESIGN.md`, "C2B.2B".
